...
@news_router.get("/{news_id}", response_model=NewsReadItem)
async def get_news_item(news_id: int, db: AsyncSession = Depends(get_db)) -> News:
    return await NewsService.get_news_item(db, news_id)

@news_router.post("", response_model=NewsReadItem)
async def create_news_item(news_item: NewsCreate, db: AsyncSession = Depends(get_db)) -> News:
...
@news_router.put("/{news_id}", response_model=NewsReadItem)
async def update_news_item(news_id: int, news_item: NewsCreate, db: AsyncSession = Depends(get_db)) -> News:
    return await NewsService.update_news_item(db, news_id, news_item.dict())

@news_router.patch("/{news_id}", response_model=NewsReadItem)
async def partial_update_news_item(news_id: int, news_item: NewsCreate, db: AsyncSession = Depends(get_db)) -> News:
    return await NewsService.update_news_item(db, news_id, news_item.dict(), partial=True)



@categories_router.get("", response_model=Sequence[CategoryRead])
...
async def update_category(category_id: int, category: CategoryCreate, db: AsyncSession = Depends(get_db)) -> Category:
...
async def partial_update_category(category_id: int, category: CategoryCreate, db: AsyncSession = Depends(get_db)) -> Category:
